...
public class spawner_L3 : MonoBehaviour
{
    ...
    { if(Timer.clock<45){
        if(t>=3){
            ...
            if (rand2>=4){
                    GameObject mutant=Instantiate(mutants[rand], new Vector3(12, 0, 0), Quaternion.identity);
                    //Instantiate(red_circle,new Vector3(12,0.87f,0),Quaternion.identity);

                    mutant.GetComponent<bottoni>().circ=0;
            }
            else{
                GameObject animal=Instantiate(animals[rand], new Vector3(12, 0, 0), Quaternion.identity);
